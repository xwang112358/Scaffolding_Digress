...
def main(cfg: DictConfig):
    ...
    metrics = MoleculeValidator(atom_decoder=atom_decoder)
    metrics_dict = metrics.process_molecules(generated_graphs)
    relax_valid_indices = metrics_dict['relaxed_valid_indices']
    print(f'relax_valid_indices: {len(relax_valid_indices)}')
    valid_indices = metrics_dict['valid_indices']
    ...
    valid_label_list = []
    valid_generated_graphs_list = []
    for i in relax_valid_indices:
        valid_label_list.append(selected_data[i].y.item())
        valid_generated_graphs_list.append(generated_graphs[i])

    # store the valid graphs

    valid_pyg_graphs = convert_graphs(valid_generated_graphs_list, valid_label_list)
    
    path = f'/home/allenwang/scaffold-aware/Scaffolding_Digress/augment_valid_pyg_graphs_labels/{name}_{split_scheme}_{ratio}_augment_valid_pyg_graphs_labels.pt'
    torch.save(valid_pyg_graphs, path)
